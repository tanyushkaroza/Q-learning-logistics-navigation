...
class MatricesForTrain:
    ...
    def get_params(self, size: int) -> Dict[str, float]:
        configs = {5: 0.35, 7: 0.35, 10: 0.35}
        density = configs.get(size, 0.35)
        return {
            'density': density,
            'step': -1,
            'barrier': -10,
    ...
    def generate_training_set(self, sizes: List[int] = [5, 7, 10],
                              samples_per_size: int = 100,
                              verbose: bool = False) -> List[np.ndarray]:
        training_set = []
        if verbose:
            print(f"Генерация матриц (seed={self.random_state})")

        for size in sizes:
            for _ in range(samples_per_size):
                training_set.append(self.generate_matrix(size))
            if verbose:
                print(f"  ✅ Размер {size}x{size}: {samples_per_size} шт.")

        return training_set

    def save_matrices_csv(self, matrices: List[np.ndarray], filename: str = "train_matrices.csv") -> str:
        os.makedirs("matrices_data", exist_ok=True)
        filepath = os.path.join("matrices_data", filename)

        with open(filepath, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(["#", f"Обучающие матрицы {len(matrices)} шт."])
            writer.writerow(["#", "Формат: step=-1 (проход), barrier=-10 (препятствие), goal=100 (цель)"])
            writer.writerow([])

            for idx, matrix in enumerate(matrices):
                writer.writerow([f"Matrix_{idx}", f"size={matrix.shape[0]}x{matrix.shape[0]}"])
                for row in matrix:
    ...
    def generate_and_save(self, sizes=[5, 7, 10], samples_per_size=100,
                          verbose=False) -> List[np.ndarray]:
        matrices = self.generate_training_set(sizes, samples_per_size, verbose=verbose)
        self.save_matrices_csv(matrices, filename="train_matrices.csv")
        return matrices

    def load_matrices_csv(self, filename: str = "train_matrices.csv") -> List[np.ndarray]:
        filepath = os.path.join("matrices_data", filename)
        matrices = []
        current_matrix = []
            ...
            for row in reader:
                if not row:
                    if current_matrix:
                        ...
                        current_matrix = []
                elif row[0].startswith('Matrix_') or row[0].startswith('#'):
                    continue
                else:
                    try:
